...
export default async function handler(req, res) {
  ...
  // Rough payload size guard — Vercel functions cap request body around 4.5MB
  try {
    const approxBytes = JSON.stringify(messages).length;
    if (approxBytes > 4_200_000) {
      return res.status(413).json({ error: 'Attachments too large. Try fewer or smaller images.' });
    }
  } catch {}
